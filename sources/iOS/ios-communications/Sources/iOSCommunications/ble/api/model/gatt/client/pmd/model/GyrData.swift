...
public class GyrData {
    ...
    static func parseDataFromDataFrame(frame: PmdDataFrame) throws -> GyrData {
        if (frame.isCompressedFrame) {
            ...
            default: throw PmdDataParseError(message: "Compressed FrameType: \(frame.frameType) is not supported by Gyro data parser")
            }
        } else {
            throw PmdDataParseError(message: "Raw FrameType: \(frame.frameType) is not supported by Gyro data parser")
        }
    }
    
    private static func dataFromCompressedType0(frame: PmdDataFrame) throws -> GyrData {
        guard !frame.dataContent.isEmpty else {
            throw PmdDataParseError(message: "Gyro compressed TYPE_0 dataContent is empty")
        }
