                    ...
                    .subscribe(
                        onSuccess: { content in
                            do {
                                ...
                                BleLogger.trace("getSubRecordingCount: Found \(subrecordingCount) sub-recordings for \(fileType)")
                                single(.success(subrecordingCount))
                            } catch {
                                BleLogger.error("getSubRecordingCount: Failed to parse directory: \(error)")
                                single(.failure(self.handleError(error)))
                            }
                        },
                        onFailure: { error in
                            BleLogger.error("getSubRecordingCount: Request failed: \(error)")
                            single(.failure(self.handleError(error)))
                        }
                    )
            } catch {
                BleLogger.error("getSubRecordingCount: Exception: \(error)")
                single(.failure(self.handleError(error)))
            }
            return Disposables.create()
        }
