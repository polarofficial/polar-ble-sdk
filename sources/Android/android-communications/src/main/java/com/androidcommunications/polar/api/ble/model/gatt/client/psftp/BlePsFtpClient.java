...
public class BlePsFtpClient extends BleGattBase {
    ...
    public Single<ByteArrayOutputStream> request(final byte[] header) {
        return request(header, Schedulers.newThread(), true);
    }
     ...
     * @param bypassCache false = use cache if possible, true = bypass cache<BR>
     ...
     */
    public Single<ByteArrayOutputStream> request(final byte[] header, Scheduler scheduler, @Deprecated final boolean bypassCache) {
        return Single.create(new SingleOnSubscribe<ByteArrayOutputStream>() {
            @Override
            public void subscribe(SingleEmitter<ByteArrayOutputStream> emitter) {
    ...
    public Flowable<Long> write(final byte[] header, final ByteArrayInputStream data, final Scheduler scheduler) {
        return Flowable.create(new FlowableOnSubscribe<Long>() {
            @Override
            public void subscribe(@NonNull FlowableEmitter<Long> subscriber) throws Exception{
                // block until previous operation has completed
                synchronized (pftpOperationMutex) {
                    if (pftpMtuEnabled.get() == ATT_SUCCESS) {
        ...
        }, BackpressureStrategy.LATEST).doOnSubscribe(new Consumer<Subscription>() {
            @Override
    ...
    public Flowable<BlePsFtpUtils.PftpNotificationMessage> waitForNotification(Scheduler scheduler){
        return Flowable.create(new FlowableOnSubscribe<BlePsFtpUtils.PftpNotificationMessage>() {
            @Override
            public void subscribe(@NonNull FlowableEmitter<BlePsFtpUtils.PftpNotificationMessage> subscriber) {
        ...
        },BackpressureStrategy.BUFFER).onBackpressureBuffer(100, new Action() {
            @Override
            public void run() {
                BleLogger.w(TAG,"notifications buffer full");
            }
