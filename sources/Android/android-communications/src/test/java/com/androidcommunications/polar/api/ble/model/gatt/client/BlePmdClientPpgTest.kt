...
import com.androidcommunications.polar.api.ble.model.gatt.client.BlePMDClient.PpgData
...
class BlePmdClientPpgTest {
    ...
    fun test_PPG_DataSample_Type0() {
        ...
        // Act
        val ppgData = PpgData(measurementFrame, timeStamp, frameType.toInt())
    ...
    fun test_PPG_DataSample_Delta() {
        ...
        // Act
        val ppgData = PpgData(measurementFrame, factor, resolution, channels, timeStamp)
        ...
        Assert.assertEquals(4, ppgData.ppgSamples[0].ppgDataSamples.size)
        Assert.assertEquals(0, ppgData.ppgSamples[0].status)
        Assert.assertEquals((factor * refSample0Channel0).toInt(), ppgData.ppgSamples[0].ppgDataSamples[0])
        Assert.assertEquals((factor * refSample0Channel1).toInt(), ppgData.ppgSamples[0].ppgDataSamples[1])
        Assert.assertEquals((factor * refSample0Channel2).toInt(), ppgData.ppgSamples[0].ppgDataSamples[2])
        Assert.assertEquals((factor * refSample0Channel3).toInt(), ppgData.ppgSamples[0].ppgDataSamples[3])
        Assert.assertEquals((factor * (refSample0Channel0 + refSample1Channel0)).toInt(), ppgData.ppgSamples[1].ppgDataSamples[0])
        Assert.assertEquals((factor * (refSample0Channel1 + refSample1Channel1)).toInt(), ppgData.ppgSamples[1].ppgDataSamples[1])
        Assert.assertEquals((factor * (refSample0Channel2 + refSample1Channel2)).toInt(), ppgData.ppgSamples[1].ppgDataSamples[2])
        Assert.assertEquals((factor * (refSample0Channel3 + refSample1Channel3)).toInt(), ppgData.ppgSamples[1].ppgDataSamples[3])
        Assert.assertEquals(4, ppgData.ppgSamples[1].ppgDataSamples.size)
        Assert.assertEquals(0, ppgData.ppgSamples[1].status)
    }
}