...
class BleBattClientTest {
    ...
    fun cachedValue() {
        ...
        //Act
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[0].toByte()), status, notifying)
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[1].toByte()), status, notifying)
    ...
    fun batteryNotificationReceived() {
        ...
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[0].toByte()), status, notifying)
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[1].toByte()), status, notifying)
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[2].toByte()), status, notifying)
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[3].toByte()), status, notifying)
        bleBattClient.processServiceData(characteristic, byteArrayOf(deviceNotifyingBatteryData[4].toByte()), status, notifying)
