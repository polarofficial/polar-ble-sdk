...
class BDBleApiImpl private constructor(context: Context, features: Set<PolarBleSdkFeature>) : PolarBleApi(features), BlePowerStateChangedCallback {
    ...
    override fun removeOfflineRecord(identifier: String, entry: PolarOfflineRecordingEntry): Completable {
        ...
        val fsType = getFileSystemType(session.polarDeviceType)
        return if (fsType == FileSystemType.SAGRFC2_FILE_SYSTEM) {
            /// Fetch the sub recording count to iterate over all subrecording and delete them one by one.
            ///
            /// Context: Offline recordings are stored in batches of sub recordings. For example,
            /// HR recording /U/0/20241203/R/094150/HR.REC may be actually stored as the following
            /// sub recording files:
            /// - /U/0/20241203/R/094150/HR0.REC,
            /// - /U/0/20241203/R/094150/HR1.REC,
            /// - ...
            ///
            /// Note that the file /U/0/20241203/R/094150/HR.REC doesn't really exist on the device,
                    ...
                    if (count == 0 || entry.path.contains(Regex("""(\D+)(\d+)\.REC"""))) {
                        ...
                        Observable.fromIterable(0 until count)
                            ...
                            .concatMap { subRecordingIndex ->
                                ...
                                BleLogger.d(TAG, "Remove offline record from device $identifier at path ${entry.path}: removing sub recording $recordingPath")
