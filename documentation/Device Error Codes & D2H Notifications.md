...
| `0x10` | 16 | `PMD_ERROR_INVALID_SOURCE_MEASUREMENT_TYPE` | The source measurement type specified for the derived measurement is incompatible: the requested derivation cannot be applied to that source type. | Call 'requestDerivedMeasurementGroupIds() with source type to retrieve available derived measurement settings group identifiers for a source measurement type. Call 'requestDerivedMeasurementSettingsGroup() with valid group identifier to retrieve actual supported derived measurement settings for the source measurement type.  Call 'startDerivedOfflineRecording() for derived measurement offline recording with valid returned derived measurement settings.  |
| `0x11` | 17 | `PMD_ERROR_INVALID_SOURCE_MEASUREMENT_RATE` | The sample rate specified for the source measurement in the derived measurement request is not supported by the device in the current state. | Call 'requestDerivedMeasurementGroupIds() with source type to retrieve available derived measurement settings group identifiers for a source measurement type. Call 'requestDerivedMeasurementSettingsGroup() with valid group identifier to retrieve actual supported derived measurement settings for the source measurement type, including source measurement sample rate.  Call 'startDerivedOfflineRecording() for derived measurement offline recording with valid returned derived measurement settings.  |
| `0x12` | 18 | `PMD_ERROR_INVALID_DERIVED_MEASUREMENT_SETTINGS_GROUP` | The settings group ID in the start-measurement or Get Derived Measurement Settings Group request is not recognised or not supported by the device. | Call 'requestDerivedMeasurementGroupIds() with source type to retrieve available derived measurement settings group identifiers for a source measurement type.  Call 'startDerivedOfflineRecording() for derived measurement offline recording with a valid group identifier. |
